...
var index = (baseUrl, config={})=>{
    ...
    const _fetch=(method,body, parts)=> {
        let u = url(parts), m = _config(method, body);
        return fetch(u, m)
            .then(async response =>{
    ...
    //ajout samszo
    const _getQuery=([part1,conditions])=>url(['records', ...part1].concat(conditions ? [query(conditions)] : []));

    const _syncRequest=(q)=>{
        const request = new XMLHttpRequest();
        request.open('GET', q, false);  // `false` makes the request synchronous
        for (const h in config.headers) {
            request.setRequestHeader(h,config.headers[h]);
        }
        request.send(null);        
    ...
    const _syncPost=(d,u)=>{
        const request = new XMLHttpRequest();
        request.open('POST', u, false);  // `false` makes the request synchronous
        for (const h in config.headers) {
            request.setRequestHeader(h,config.headers[h]);
        }
    ...
    const _syncPut=(d,u)=>{
        const request = new XMLHttpRequest();
        request.open('PUT', u, false);  // `false` makes the request synchronous
        for (const h in config.headers) {
            request.setRequestHeader(h,config.headers[h]);
        }
        request.send(JSON.stringify(d));        
    ...
    return {
        ...
        //ajout samszo
        query:(table,conditions={})=>_getQuery([[table],conditions]),
        syncList:(table,conditions={})=>_syncRequest(_getQuery([[table],conditions])),
        syncRead:(table,ids,conditions={})=>_syncRequest(_getQuery((ids?[[table,join()(ids)],conditions]:[[table]]))),
        syncCreate:(table,data)=>_syncPost(data,url(['records',table])),
        syncUpdate:(table,data,id)=>_syncPut(data,url(['records',table,id])),
        stats:(s,t,id)=>_fetch('GET',null,['apiStats',s,t,id]),
        //fin ajout samszo
    };
};
